...
class TestCmdIt140(object):
    ...
    def test_output_base_command(self):
        response = cmd_it140.execute(self.cmd, self.uid, self.bot)
        response_topics = [x.replace('`', '').replace('-', '').strip() for x in response[0].split('\n')[2:]]

        assert response[0].startswith("Here is a list of valid IT-140 topics.  Proceed by entering: `<@{}> it140 topic`.".format(self.uid))
        assert len(response_topics) == len(cmd_it140.data) - 1  # because of hidden IT140 command
